...
while True:
    ...
    for i in contours:
        ...
        x_over_w = (np.arctan(((deltax/w)*22)/distance))*180/(np.pi)
        anglex = int(np.arctan(((9132.7 * (w**-0.47))/distance)))
        b = "anglex =", x_over_w
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(frame,str(b),(15,105), font, .5,(255,255,255),2,cv2.LINE_AA)
        j+=10
        if b == ord('q'):
            break
